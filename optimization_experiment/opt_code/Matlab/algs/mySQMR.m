function [x,flag,relres,iter,resvec] = mySQMR(A,b,tol,maxit,M1,M2,x0,varargin)
...
if existM1
   if isequal(m1type,'matrix')
      ...
      t = iterapp('mldivide',m1fun,m1type,m1fcnstr,r,varargin{:});
   end
   if isinf(norm(t,inf))
      flag = 2;
      relres = normr/n2b;
...
if existM2
   if isequal(m2type,'matrix')
      ...
      q = iterapp('mldivide',m2fun,m2type,m2fcnstr,t,varargin{:});
   end
   if isinf(norm(q,inf))
      flag = 2;
      relres = normr/n2b;
...
for i = 1 : maxit
   if isequal(atype,'matrix')
      ...
      t = iterapp('mtimes',afun,atype,afcnstr,q,varargin{:});         
   end
   sigma = q' * t;
   ...
   if rho == 0
       ...
       if existM2
           if isequal(m2type,'matrix')
               ...
               u = iterapp('mldivide',m2fun,m2type,m2fcnstr,t,varargin{:});
           end
           if isinf(norm(u,inf))
               flag = 2;
               break
