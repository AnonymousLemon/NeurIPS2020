function [alpha,itrLS] = lineSearchWolfeStrong(objFun, xk, fk, gk, pk, alpha0, beta1, beta2, linesearchMaxItrs)
phi_0 = fk;
d_phi_0 = dot(pk,gk);
...
alpha = alpha0;
itrLS = 0;
zoomFun = @(alpha_low,alpha_high,itrLS)zoom(objFun, xk, fk, gk, pk, beta1, beta2, linesearchMaxItrs, alpha_low,alpha_high,itrLS);

while itrLS < linesearchMaxItrs
    [phi_alpha , g_alpha] = objFun(xk+alpha*pk);
    d_phi_alpha = dot(pk,g_alpha);
    if ( phi_alpha > phi_0 + beta1*alpha*d_phi_0 ||  ( ( phi_alpha  >= phi_alpha_prev ) && itrLS > 0 ) )
