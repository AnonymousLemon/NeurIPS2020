function [x,flag,relres,iter,Miter,QLPiter,relAbres,relAres,...
    Anorm,Acond,xnorm,Axnorm,resvec,Aresvec] = ...
    myMINRESQLP(A,b,rtol,maxit,M,shift,maxxnorm,Acondlim,TranCond,show)
...
%   -3 (beta2=0)  B and X are eigenvectors of (A - SHIFT*I).
%   -1 (beta1=0)  B = 0.  The exact solution is X = 0, i.e., B is in the null space of A.
%    0 Relative residual ||Ax - b||/||b|| <= rTol has achieved
...
u_QLP    = 0;      ul_QLP   = 0;
relAbres   = rnorm / (beta1 + 1e-50);       % Safeguard for beta1 = 0
relresl  = 0;
relAresl = 0;
x        = zeros(n,1);
xl2      = x;
...
last  = 'Exit minresqlp.  ';
msg=[' beta2 = 0.  b and x are eigenvectors                   '   % -3
    ' beta1 = 0.  The exact solution is  x = 0               '   % -1
...
while flag == flag0 && iter < maxit
    ...
    if ~precon
        ...
        if iter == 1       % Something special can happen
            if betan == 0    % beta2 = 0
                if alfa == 0   % alfa1 = 0
                    flag = -1;    % Ab = 0 and x = 0         ("A" = (A - shift*I))
                    break
                else
                    flag = -3;   % Ab = alfa1 b,  x = b/alfa1, an eigenvector
                    x    = full(b)/alfa;
                    break
                end
            end
    ...
    rnorml   = rnorm;     relresl = relAbres;
    if (flag ~= 9)
        rnorm = phi;
    end
    relAbres   = rnorm / (Anorm*xnorm + beta1);
    rootl    = norm([gbar; dltan]);
    Arnorml  = rnorml*rootl;
    relAresl = rootl / Anorm;
    relres = termFun(x);%rnorm / norm(b);
    ...
    if (flag == flag0) || (flag == 9)
        ...
        %     if relAbres <= rtol    , flag = 1; end  % Good enough Ax=b solution
        if relres   <= rtol    , flag = 0; end  % Small enough ||Ax-b||/||b||
    end
    ...
    if flag == 2 || flag == 4 || (flag == 6 && likeLS) || flag == 7   % Possibly singular
        iter  = iter - 1;
        Acond = Acondl;   rnorm = rnorml;   relAbres = relresl;
    else
        if ~isempty(resvec)
            resvec(iter+1) = rnorm;
...
if show
    if rnorm > realmin
        fprintf('%7g %10.2e %10.2eD%10.2e %10.2eD%10.2e %10.2e %10.2e\n\n',...
            iter, rnorm, Arnorm, relAbres, relAres, Anorm, Acond, xnorm)
    else
        fprintf('%7g %10.2e %10.2eD%10.2e %10.2e             %10.2e %10.2e\n\n',...
            iter, rnorm, Arnorm, relAbres,          Anorm, Acond, xnorm)
    end
end
