function [f,g,H] = invexFunReg05(x,A, regFun)
if nargin==0
    tic;
    runTest
    toc;
...
f3 = log(Ax3); %0.5*(Ax3+1).^2+(Ax3+1) + 1 + (2*sin((2)/2)+(2) - (0.5*(2).^2+(2) + 1));
...
idx1 = idx1 +0; idx1(idx1 == 1) = 1;
idx2 = idx2 +0;idx2(idx2 ==1) = -1;
idx3 = idx3 +0; idx3(idx3 ==1) = -1./(Ax3.^2);

if nargout >= 3
    H = @(v) A'*( ( idx3 + idx1 + idx2 ) .* (A*v) )/n + Hv_reg(v);
end
...
% n = 10;
% A = randn(n,p);
% derivativeTest(@(x) invexFunReg05(x,A),randn(p,1))
...
for i = 1:length(x1)
    [f1(i),g1(i),Hv1] = invexFunReg05(x1(i),1);
    H1(i) = Hv1(1);
end
...
for i = 1:length(x2)
    [f2(i),g2(i),Hv2] = invexFunReg05(x2(i),1);
    H2(i) = Hv2(1);
end
...
for i = 1:length(x3)
    [f3(i),g3(i),Hv3] = invexFunReg05(x3(i),1);
    H3(i) = Hv3(1);
end
...
plot(-4, invexFunReg05(-4,1), 'g.', 'MarkerSize',27);
...
dir_name = './results/exampleFunction05';
